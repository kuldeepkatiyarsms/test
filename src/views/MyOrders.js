...
import { Container, Content, List, ListItem, Col, Tab, Tabs, TabHeading, Icon, Thumbnail } from 'native-base';
...
class MyOrders extends Component<Props> {
  ...
  mybooking_details = (data) => {
    this.props.navigation.navigate('MyBookingDetails',{ data : data });
  }
  ...
  my_orders = async () => {
    ...
    .then(async response => {
      ...
      this.setState({ isLoding : false ,api_status:1,orders: response.data.result.orders,booking_requests: response.data.result.booking_requests});
      await this.props.serviceActionSuccess(response.data)
      if(response.data.result.length == 0){
        this.setState({ current_status: 'No data found'});
      }else{
  ...
  update_rating = (data) => {
    this.props.navigation.navigate('Rating',{ data : data });
  }

  get_status(booking_status,booking_request_status,booking_status_name,booking_request_status_name){
      if(booking_status){
        return(
  ...
  render() {
    Moment.locale('en');
    const { isLoding, data, bookings, } = this.props
        ...
        <Tabs tabBarUnderlineStyle={styles.order_style1} >
          <Tab heading={ <TabHeading style={styles.order_style2}><Icon style={styles.order_style3} name="pulse" /><Text style={styles.order_style4}>Appointments</Text></TabHeading>}>
            <Content>
              <List>
                <FlatList
                  ...
                  renderItem={({ item,index }) => (
                    <ListItem onPress={() => this.mybooking_details(item)}>
                      <Col style={styles.order_style5}>
                        <Thumbnail square source={{uri: img_url + item.profile_image}} />
                      </Col>
                      <Col style={styles.order_style6} >
                        ...
                        {this.get_status(item.booking_status, item.booking_request_status, item.booking_status_name, item.booking_request_status_name)}  
                      </Col>
              ...
              { bookings.length == 0 && this.state.api_status == 1 &&
                <View>
              ...
              <List>
                {data.map((row, index) => (
                  <ListItem onPress={() => this.myorders_details(row)} >
                    <Col style={styles.order_style19}>
                      <ProgressCircle
                    ...
                    <Col style={styles.order_style22} >
                      ...
                      {row.status == 8 && row.rating_update_status == 0 &&
                        <Btn
                          ...
                          onPress={this.update_rating.bind(this,row)}
                        />
                      }
                    </Col>
                    <Col>
