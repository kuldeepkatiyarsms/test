...
import { Content, Container, Left, Row, Col, Icon, Thumbnail} from 'native-base';
...
export default class App extends Component<props> {
  ...
  appoinment_details = (type) => {
    if(type == 1){
      this.props.navigation.navigate('CreateAppointment', { id: this.state.data.id, price_per_conversation: this.state.data.booking_settings?this.state.data.booking_settings.online_booking_fee:'', type:type});
    }else{
      this.props.navigation.navigate('CreateAppointment', { id: this.state.data.id, price_per_conversation: this.state.data.booking_settings?this.state.data.booking_settings.direct_appointment_fee:'', type:type});
    }
  }
    ...
    return(
      <Container>
          ...
          <View style={styles.doc_style1}>
            <View style={styles.doc_style2}>
              <View style={styles.doc_style3}>
                <TouchableOpacity style={styles.doc_style4} onPress={this.handleBackButtonClick} activeOpacity={1} >
                <Icon onPress={this.handleBackButtonClick} style={styles.doc_style5} name='arrow-back' />
                </TouchableOpacity>
                <View style={styles.doc_style6} />
                <Text style={styles.doc_style7}>Dr.{this.state.data.doctor_name}</Text>
                ...
                <Text style={styles.doc_style11}>
                  ...
                  {parseInt(this.state.data.overall_rating) > 0 &&
                      <Rating
                        ratingCount={5}
                        startingValue={this.state.data.overall_rating}
                        imageSize={12}
                        readonly={true}
                      />
                  }
            ...
            <View style={styles.doc_style13}>
              <View style={styles.doc_style14}>
                ...
                <Thumbnail large source={{ uri : img_url + this.state.data.profile_image}} />
        ...
        <Content padder>
          ...
          {this.state.data.booking_settings&&this.state.data.booking_settings.online_booking_status == 1 &&
          <View style={{ borderTopLeftRadius:10, borderTopRightRadius:10, borderColor:'grey', borderWidth:1}}>
            <View style={{ width:'100%', backgroundColor:colors.theme_light, borderTopLeftRadius:10, borderTopRightRadius:10, padding:10 }}>
              <Text style={{ fontFamily:font_title, fontSize:15, color:colors.theme_fg_two}}>Online consultation - {global.currency}{this.state.data.booking_settings.online_booking_fee}</Text>
            </View>
            <View style={{ width:'100%', padding:10 }}>
              ...
              <Text style={{ fontFamily:font_description, fontSize:13, color:colors.grey}}>Consulting duration : <Text style={{ fontWeight:'bold', color:colors.theme_fg_two}}>{this.state.data.booking_settings.online_booking_time} min</Text></Text>
            </View>
            <Button
              title="Book now"
              type="outline"
          ...
          <View style={{ margin:10 }} />
          {this.state.data.booking_settings&&this.state.data.booking_settings.direct_appointment_status == 1 &&
          <View style={{ borderTopLeftRadius:10, borderTopRightRadius:10, borderColor:'grey', borderWidth:1}}>
            <View style={{ width:'100%', backgroundColor:colors.theme_light, borderTopLeftRadius:10, borderTopRightRadius:10, padding:10 }}>
